...
def detect_hardware_and_tier() -> tuple[dict, str]:
    ...
    try:
        ...
        logger.warning("hardware_detector non disponible — détection ignorée")
        return {}, "light"
    except Exception as e:
        logger.warning("Erreur lors de la détection matérielle : %s", e)
        return {}, "light"
    ...
    try:
        ...
        for agent_name in AGENT_REGISTRY:
            try:
                ...
                logger.info("  Enregistré : %s", agent_name)
            except Exception as e:
                logger.error("  Échec enregistrement %s : %s", agent_name, e)
...
class LuymasLauncher:
    ...
    def run(self) -> None:
        ...
        # ── Pre-flight checks ────────────────────────────────────────────
        logger.info("Luymas AI v%s", VERSION)
        logger.info("─" * 48)
        ...
        try:
            ...
            if max_m == -1:
                ...
                logger.info("Mode RAM : Strict (%s) — UN SEUL modèle à la fois", tier)
            else:
        ...
        logger.info("")
        logger.info("Luymas AI est en cours d'exécution")
